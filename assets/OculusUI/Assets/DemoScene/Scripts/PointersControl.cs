...
public class PointersControl : MonoBehaviour {
    ...
    private UI_OVRInputModule _inputModule;
    private UI_OVRInputModule inputModule
    {
        get 
        {
            if (_inputModule == null)
            {
                _inputModule = EventSystem.current.currentInputModule as UI_OVRInputModule;
            }
            return _inputModule;
        }
    }
    ...
    {
        FindObjectOfType<UI_OVRInputModule>().matchNormalOnPhysicsColliders = on;
    }
    public void SetHideGazepointerByDefault(bool hide)
    {
        UI_OVRGazePointer.instance.hideByDefault = hide;
    }
    public void SetOnlyDimCursorWhenMouseActive(bool dim)
    {
        UI_OVRGazePointer.instance.dimOnHideRequest = dim;
    }
