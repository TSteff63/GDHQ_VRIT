...
[RequireComponent(typeof(UI_OVRRaycaster))]
public class UI_OVRMousePointer : MonoBehaviour {
    ...
    public GameObject pointerObject
    {
        get
        {
            return GetComponent<UI_OVRRaycaster>().pointer;
        }
    }


    UI_OVRRaycaster raycaster;
    void Awake()
    {        
        raycaster = GetComponent<UI_OVRRaycaster>();       
#if UNITY_ANDROID
	...
	void Update () 
    {
        ...
        if (hideGazePointerWhenActive && HasMovedRecently() && raycaster.IsFocussed())
            UI_OVRGazePointer.instance.RequestHide();
